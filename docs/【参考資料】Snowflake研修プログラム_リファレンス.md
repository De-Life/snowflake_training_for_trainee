...
| Japanese Language Pack for Visual Studio Code | VSCodeの表示を日本語にする |
...
- **Lambda 関数の依存**：メールデータを JSON 形式で Snowflake の `VARIANT` カラムに保存
- **Terraform 変数**：Terraform 依存を記述（`.tfvars.json`）
- **API 応答**：CloudFormation テンプレート応答を JSON 形式で返却
...
- **dbt 設定**：`dbt_project.yml`、`sources.yml`、`profiles.yml`
- **GitHub Actions パイプライン**：`.github/workflows/main.yml`
- **CloudFormation テンプレート**：AWS 依存記述用
- **AWS Lambda を起動して打ち出す dbt パイプライン等**：設定ファイルの書き方

### 3.3 XML (eXtensible Markup Language)

**概説**

XML はタグを使って複雑な複合データ構造を表現できる。SOAP API や Office ファイル（`.xlsx`、`.docx`）のベース形式。
...
- **EML ファイル（メール）**：Lambda 関数で置かれるメールを XML 際やき。MIME 構造を採用
- **API 応答（古いシステム）**：SOAP を使用しているシステムとの連携
...
### おまけ
さらなる解説を希望する方は[こちら](../knowledges/Git_Github_解説簡易版.md)
...
AWS のIaC（Infrastructure as Code）サービス。YAML または JSON 形式のテンプレートでリソースを記述し、スタック（リソースの集合）を一括作成・更新できる。

...
## ANCHORとは
営業課題の解決を目的に開発された、Webアプリケーションです。Outlookで日々大量に受信するメールを自動取得し、AWSおよびSnowflake上でのデータ加工を経て、便利なメール検索アプリとして表示しています。
...
5. [HCP Terraform環境構築（5h）](#chapter-5)

6. [GitHub環境構築（4h）](#chapter-6)

7. [TerraformによるSnowflakeオブジェクトの定義（5h）](#chapter-7)

8. [dbtによるデータ加工（8h）](#chapter-8)

9. [Streamlitアプリケーションの実装（8h）](#chapter-9)

10. [レポート作成（8h）](#chapter-10)
...
- ガイドライン(json)に沿って、課題を満たす内容の構成図XMLをAIに出力させます。
...
今登場した用語については[Snowflake研修リファレンス-AWS環境構築](【参考資料】Snowflake研修プログラム_リファレンス.md#8-aws-環境構築)を参照してください。
...
- 課題①・②を実施します。課題②で作成したテキストファイルは、成果物フォルダに提出してください。
...
### ＜解説＞
②
今回設定している変数は、Snowflake側で読み込むS3バケットのURLと、自身のユーザー名です。
S3バケットURIは、S3バケット(raw_emails)のURIをAWSコンソールでコピーして入力することとなります。
また、snowflake_aws_role_arnには、AWS IAM のsnowflake-storage-integration-roleを参照し、ARNの値をコピーして入力することとなります。(arn:aws:iam::875180007397:role/snowflake-storage-integration-roleのような値になっていればOKです)
...
本章では、「リポジトリ」や「クローン」など、GitおよびGithubの用語が登場します。そちらについては[Snowflake研修リファレンス2～4章](【参考資料】Snowflake研修プログラム_リファレンス.md#2-git)をご参照ください。
また、本章でGitHubの環境構築を実施する目的は、今後の6章・7章の内容をCI/CDで実行することにあります。CI/CDについては、[Snowflake研修リファレンス7章](【参考資料】Snowflake研修プログラム_リファレンス.md#7-cicd-パイプライン)をご参照ください。
...
主要なファイルの解説、各ファイル間の依存関係については[Snowflake研修リファレンス5章](【参考資料】Snowflake研修プログラム_リファレンス.md#5-terraform)をご参照ください。
また、Snowflake側の理解も必要となります。[Snowflake研修リファレンス9章](【参考資料】Snowflake研修プログラム_リファレンス.md#9-snowflake-環境構築)をご参照し、どのオブジェクトがtfファイルで定義されているか理解することをおすすめします。

### ＜手順概要＞
- 課題①～③に沿って、対応するソースコードをローカルにて編集します。
...
主要なファイルの解説、各ファイル間の依存関係については[Snowflake研修リファレンス6章](【参考資料】Snowflake研修プログラム_リファレンス.md#6-dbt)をご参照ください。

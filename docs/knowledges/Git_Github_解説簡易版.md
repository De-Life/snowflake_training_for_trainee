...
> **なぜCドライブ？**
> 
> **「OneDriveがパスに含まれることを回避するため」** です。
> OneDriveのフォルダ名として、日本語を含める企業が非常に多いです。
> OneDrive内にフォルダを作成すると、
> `C:\Users\hoge\OneDrive - De-Life株式会社 \ Documents \ Git`
> みたいになります。
> しかし、**開発では日本語に対応しないツールをよく使用する**ため、「こんな文字読めません」といったエラーが発生する可能性があります。
> 
> Windowsの場合、あまり考えないと大体OneDriveの中にフォルダが作られます。
> 「デスクトップ」も「ドキュメント」も「ピクチャ」も、OneDriveの配下です。
> そのため、Cドライブ直下にGitリポジトリを格納するフォルダを作成することが無難になります。
> つまり、Gitリポジトリ内のファイルパスに日本語が含まれなければ、OKです。
